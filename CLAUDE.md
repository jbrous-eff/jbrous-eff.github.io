...
The user's charts are made outside this repo — **R/ggplot2, in `C:\Users\jbrou\Documents\Projects\pro-sports\WNBA\scripts`** for the sports pieces; the business charts are matplotlib, generated from a redacted local source outside this repo (the client is not named anywhere in this public repo; the source path is in Claude's memory; never regenerate from live client files) — but they're the main content of the site, so the color system is recorded here. Gray `#6a6a67` and bone are **chrome only** (axes, grids, reference lines, annotations), never a series/indicator color — data marks take the categorical set or the judgment pair (user call, 2026-08-06). Fixed roles, never reused for anything else:
